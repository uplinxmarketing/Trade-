...
const AITradingAgent = ({ selectedCoins, prices, binanceConnected, onConnectBinance, onCoinsChange, onStateChange, onLiveModeDetected }: AITradingAgentProps) => {
  ...
  const handleForceSell = useCallback(async (pos: OpenPosition) => {
    ...
    try {
      if (isServerMode) {
        // Server mode: delegate to VPS bot's force-sell endpoint
              ...
              <>
                <div className="space-y-2">
                  {(showAllPositions ? positions : positions.slice(0, 5)).map(pos => {
                    const cur = (isServerMode && pos.current_price) ? pos.current_price : (prices[pos.symbol] ?? pos.avg_entry_price);
                    const pnl = (cur - pos.avg_entry_price) * pos.quantity;
                    const pct = pos.avg_entry_price > 0 ? ((cur - pos.avg_entry_price) / pos.avg_entry_price) * 100 : 0;
                    return (
                      <div key={pos.symbol} className="flex items-center justify-between bg-muted/20 rounded px-3 py-2">
                        <div className="flex items-center gap-2">
                          <div className={`w-1.5 h-1.5 rounded-full ${pnl >= 0 ? 'bg-gain' : 'bg-loss'}`} />
                          <div>
                            <p className="text-xs font-bold">{pos.symbol.replace('USDT','')}</p>
                            <p className="text-[9px] text-muted-foreground">{pos.quantity.toFixed(6)} @ {pos.avg_entry_price.toFixed(4)}</p>
                          </div>
                        </div>
                        <div className="text-right">
                          <p className={`text-xs font-mono font-bold ${pnl >= 0 ? 'text-gain' : 'text-loss'}`}>
                            {pnl >= 0 ? '+' : ''}{pnl.toFixed(2)}
                          </p>
                          <p className={`text-[9px] font-mono ${pct >= 0 ? 'text-gain' : 'text-loss'}`}>{pct >= 0 ? '+' : ''}{pct.toFixed(2)}%</p>
                        </div>
                        <button
