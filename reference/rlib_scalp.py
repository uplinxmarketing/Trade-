...
def build(tag, step=3, cap=864, v2=False):
    ...
    NF = 29 if v2 else 26
    ARR = {}; Xs = []; Ws = []; W12s = []; W3s = []; Fz = []; Is = []; Cs = []; As = []
    warm = 2100 if v2 else 620
    return _build2(Z, FV, N, coins, HS, HC, MED1H, MED24, BREADTH1H, BREADTH24,
                   BTC15, BTC1H, BTC7D, NF, ARR, Xs, Ws, W12s, W3s, Fz, Is, Cs, As, warm, step, cap, v2, tag)
...
def _build2(Z, FV, N, coins, HS, HC, MED1H, MED24, BREADTH1H, BREADTH24, BTC15, BTC1H, BTC7D,
            NF, ARR, Xs, Ws, W12s, W3s, Fz, Is, Cs, As, warm, step, cap, v2, tag):
    for ci, c in enumerate(coins):
        ...
        for k in range(1, cap + 1):
            ...
            gain = peak / entry - 1
            armed |= act & ((gain >= 0.8 * Rr) | (gain >= EXIT_ARM_ABS))
            befl |= act & ((h / entry - 1) >= bearm)
            tr_ = np.maximum(peak * (1 - atrp_e), entry * (1 + gain * (1.0 - EXIT_GIVEBACK)))
        ...
        W12 = done & (bars <= 12) & (ret >= 0.0009)
        W3 = done & (bars <= 3) & (ret >= 0.0009)
        FRZ = opn & (cl[idx + cap] < entry)
        Xs.append(F[idx]); Ws.append(WIN4); W12s.append(W12); W3s.append(W3); Fz.append(FRZ)
    ...
    out = dict(X=X, W4=np.concatenate(Ws), W12=np.concatenate(W12s), W3=np.concatenate(W3s), FZ=np.concatenate(Fz), IDX=np.concatenate(Is),
...
def simdeep(Y, ci, i, atrp, entry_mode='close', slip=0.0, rt=RT_DEF):
    key = (id(Y), ci, i, entry_mode, slip, rt, EXIT_ARM_ABS, EXIT_GIVEBACK)
    ...
    for j in range(i + 1, end):
        ...
        if px > peak: peak = px
        gain = peak / entry - 1
        if not armed and (gain >= 0.8 * R or gain >= EXIT_ARM_ABS): armed = True
        if not be and (h / entry - 1) >= bearm: be = True
        if h >= tp: out = (tp / entry - 1 - rt, j - i); break
        if armed:
            tr_ = max(peak * (1 - atrp), entry * (1 + gain * (1.0 - EXIT_GIVEBACK)))
            if l <= tr_:
                net = tr_ / entry - 1 - rt
