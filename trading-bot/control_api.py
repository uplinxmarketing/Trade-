...
def _get_positions():
    try:
        ...
        for p in pos:
            ...
            out.append({
                **p,
                # avg_entry_price alias — frontend reads this field name
...
def api_force_sell(symbol: str, req: Optional[ForceSellRequest] = None):
    ...
    try:
        ...
        # Enforce breakeven floor — NEVER execute a sell below entry + fees.
        # This prevents force-sells at a loss when prices are stale or the user
        # taps sell while the coin is temporarily dipping below entry.
        from trade_engine import _breakeven_mult
        entry = pos.get("entry_price", 0)
        breakeven_floor = round(entry * _breakeven_mult, 8) if entry else 0
        if breakeven_floor and price < breakeven_floor:
            return {
                "ok": False,
                "error": (
                    f"{sym} current price ${price:.4f} is below breakeven "
                    f"${breakeven_floor:.4f}. Position is not yet profitable. "
                    f"Stop bot and use force-sell only after price exceeds ${breakeven_floor:.4f}."
                ),
                "price": price,
                "breakeven": breakeven_floor,
            }
