...
def _get_positions():
    try:
        ...
        for p in pos:
            ...
            # (e.g. coins bought before current WS session, or coins removed from watchlist)
            if not price:
                try:
            ...
            out.append({
                **p,
                "avg_entry_price": entry,          # frontend reads this field name
                "current_price":   price,
...
def api_force_sell(symbol: str, req: Optional[ForceSellRequest] = None):
    ...
    sym = symbol.upper()
    try:
        from trade_engine import (get_open_positions, _positions, _positions_lock,
                                    _rebuild_pos_index, _breakeven_mult)
        from connection import client as _client
        from data_collector import prices as live_prices
        ...
        # Find position
        with _positions_lock:
            pos = next((p for p in _positions if p["symbol"] == sym), None)
        if not pos:
            return {"ok": False, "error": f"No open position for {sym}"}

        # Price: frontend hint > WebSocket > REST
        hint_price = (req.price if req else 0) or 0
        price = hint_price or live_prices.get(sym, 0)
        if not price:
            try:
        ...
        def _do_sell():
            result   = _client.order_market_sell(symbol=sym, quantity=pos["quantity"])
            fill     = result.get("fills", [{}])[0]
            sell_px  = float(fill.get("price", price))
            qty      = float(result.get("executedQty", pos["quantity"]))
            entry    = pos["entry_price"]
            pnl      = (sell_px - entry) * qty
            fee_rate = config.FEE_RATE_BNB if config.BNB_FEE_MODE else config.FEE_RATE_TAKER
            buy_fee  = entry * qty * fee_rate
            sell_fee = sell_px * qty * fee_rate
            net_pnl  = pnl - buy_fee - sell_fee

            trade = {
                "symbol": sym, "entry_price": entry, "exit_price": sell_px,
                "quantity": qty, "budget_usdt": pos.get("budget_usdt", 0),
                "buy_fee": buy_fee, "sell_fee": sell_fee, "net_profit": net_pnl,
                "mode": get_mode(), "profitable": net_pnl > 0,
                "hold_seconds": int(time.time() - datetime.fromisoformat(
                    pos["timestamp"].replace("Z","")).timestamp()) if pos.get("timestamp") else 0,
                "reason": "manual_sell",
            }
            database.save_trade(trade)
            database.delete_position(pos["id"])
            with _positions_lock:
                _positions[:] = [p for p in _positions if p["symbol"] != sym]
            _rebuild_pos_index()

            try:
                import supabase_sync
                supabase_sync.sync_sell_result_sync(trade, _get_usdt_balance())
            except Exception as _sbe:
                database.log_activity(f"Supabase sync error after force-sell {sym}: {_sbe}", "error")

            database.log_activity(
                f"Force sell: {sym} @ ${sell_px:.4f} | net={net_pnl:+.4f} USDT | {'WIN' if net_pnl>0 else 'LOSS'}",
                "info"
            )
            return sell_px, net_pnl

        executor = concurrent.futures.ThreadPoolExecutor(max_workers=1)
        future   = executor.submit(_do_sell)

        # Block briefly so the response includes accurate sell details.
        # 4 s timeout keeps the HTTP response fast even on slow Binance REST.
        try:
            sell_px, net_pnl = future.result(timeout=4)
            return {"ok": True, "symbol": sym, "price": sell_px, "pnl": net_pnl}
        except concurrent.futures.TimeoutError:
            # Sell still running in background — return immediately
            return {"ok": True, "symbol": sym, "price": price, "pnl": None, "note": "sell dispatched"}
...
# ── /api/all — single bundled endpoint for UI poll ───────────────────────────

# In-memory cache for /api/all — reduces SQLite reads on fast polls.
# Invalidated by _write_strategy_patch so Start/Stop/Settings changes are instant.
_API_ALL_CACHE: dict = {"data": None, "ts": 0.0}
_API_ALL_TTL = 1.0   # seconds

@app.get("/api/all")
def api_all():
    """
    Single endpoint that bundles status + positions + trades + signals + activity.
    Designed for the frontend's poll loop — one round-trip instead of four.
    Cached for 1 s to absorb rapid polls without hammering SQLite/Binance.
    """
    now = time.time()
    cached = _API_ALL_CACHE.get("data")
    if cached and (now - _API_ALL_CACHE["ts"]) < _API_ALL_TTL:
        return cached
    ...
    status_block = {
        ...
        "data_persistent":     database._DATA_DIR == "/data",
        "sell_monitor_alive":  _sell_monitor_alive(),
        # Settings fields (so UI can reflect current server-side config)
        ...
        "min_signals":         strategy.get("min_signals", 4),
        "strategy_notes":      strategy.get("strategy_notes", ""),
        "deploy_id":           _DEPLOY_ID,
    }

    positions_block = _get_positions()
    trades_block    = _format_trades(database.get_recent_trades(limit=200))
    signals_block   = _get_signal_snapshot()
    activity_block  = database.get_activity_log(limit=50)
    ...
    _API_ALL_CACHE["data"] = result
    _API_ALL_CACHE["ts"]   = now
    return result


def _format_trades(raw: list) -> list:
    """Normalise DB trade rows → frontend TradeRow shape."""
    out = []
    for t in raw:
        out.append({
            "id":           t.get("id", ""),
            "created_at":   t.get("created_at") or t.get("timestamp", ""),
            "symbol":       t.get("symbol") or t.get("coin", ""),
            "side":         "SELL" if t.get("exit_price") else "BUY",
            "price":        t.get("exit_price") or t.get("entry_price") or 0,
            "quantity":     t.get("quantity", 0),
            "pnl":          t.get("net_profit"),
            "reason":       t.get("reason"),
            "volume_usdt":  t.get("budget_usdt"),
            # Extra fields the dashboard uses
            "entry_price":  t.get("entry_price", 0),
            "exit_price":   t.get("exit_price", 0),
            "buy_fee":      t.get("buy_fee", 0),
            "sell_fee":     t.get("sell_fee", 0),
            "budget_usdt":  t.get("budget_usdt", 0),
            "profitable":   bool(t.get("profitable")),
            "duration_seconds": t.get("hold_seconds", 0),
            "net_profit":   t.get("net_profit"),
...
def _settings_response():
    ...
    return {
        ...
        "min_signals":         s.get("min_signals",         4),
        "strategy_notes":      s.get("strategy_notes",      ""),
    }

@app.get("/api/settings")
def api_get_settings(): return _settings_response()

@app.post("/api/settings")
def api_post_settings(body: dict = Body(...)):
    allowed = {
        "stop_loss_enabled", "stop_loss_pct",
        "take_profit_enabled", "take_profit_pct",
        "smart_hold_enabled", "trailing_stop_pct",
        "reinvest_profits", "max_positions", "min_signals",
        "strategy_notes",
    }
    patch = {k: v for k, v in body.items() if k in allowed}
    if not patch:
        return {"ok": False, "error": "No valid settings keys"}
    _write_strategy_patch(patch)
    return {"ok": True, "updated": list(patch.keys())}


# ── /api/ping — lightweight liveness check ───────────────────────────────────

@app.get("/api/ping")
def api_ping():
    return {"ok": True, "mode": get_mode(), "deploy_id": _DEPLOY_ID,
            "ts": datetime.now(timezone.utc).isoformat()}


# ── /api/debug — environment snapshot for troubleshooting ────────────────────

@app.get("/api/debug")
def api_debug():
    safe_keys = {"MODE", "PORT", "STARTING_PAPER_USDT", "RAILWAY_ENVIRONMENT",
                 "RAILWAY_SERVICE_NAME", "RAILWAY_DEPLOYMENT_ID"}
    env_snap = {k: os.environ.get(k, "") for k in safe_keys}
    return {
        "env":        env_snap,
        "mode":       get_mode(),
        "live_error": get_live_error() or None,
...
# ── /api/mode — switch trading mode at runtime ───────────────────────────────

@app.post("/api/mode")
def api_set_mode(body: dict = Body(...)):
    """
    Switch the bot between paper / testnet / live without a full redeploy.
    Body: { "mode": "paper"|"testnet"|"live", "api_key": "...", "api_secret": "..." }
    """
    import connection
    new_mode = body.get("mode", "").lower()
    if new_mode not in ("paper", "testnet", "live"):
        return {"ok": False, "error": "mode must be paper | testnet | live"}

    api_key    = body.get("api_key", "").strip()
    api_secret = body.get("api_secret", "").strip()

    if new_mode == "live" and (not api_key or not api_secret):
        return {"ok": False, "error": "api_key and api_secret required for live mode"}

    try:
        live_error = connection.switch_mode(new_mode, api_key=api_key, api_secret=api_secret)
        return {
            "ok":         True,
            "mode":       new_mode,
...
def api_futures_status():
    """Return current futures engine state — positions, balance, stats."""
    try:
        import futures_engine as _fe
        return _fe.get_status()
    except Exception as e:
        return {"error": str(e), "positions": [], "balance_usdt": 0}
...
def api_futures_start():
    try:
        import futures_engine as _fe
        _fe.set_active(True)
        database.log_activity("Futures trading started via API", "info")
        return {"ok": True, "running": True}
    except Exception as e:
        return {"ok": False, "error": str(e)}


@app.post("/api/futures/stop")
def api_futures_stop():
    try:
        import futures_engine as _fe
        _fe.set_active(False)
        database.log_activity("Futures trading stopped via API", "info")
        return {"ok": True, "running": False}
    except Exception as e:
        return {"ok": False, "error": str(e)}
...
@app.post("/api/sync")
def api_sync():
    """Manually trigger a Supabase sync (positions + balance + trades)."""
    try:
        from trade_engine import get_open_positions
        import supabase_sync
        positions = get_open_positions()
        usdt = _get_usdt_balance()
        supabase_sync.sync_all(positions, usdt)
        return {"ok": True, "synced_positions": len(positions), "usdt": usdt}
    except Exception as e:
        return {"ok": False, "error": str(e)}
...
if __name__ == "__main__":
    # Initialise DB synchronously before uvicorn starts — guarantees the schema
    # exists before any HTTP request could arrive (avoids race on first deploy).
    database.init_db()
